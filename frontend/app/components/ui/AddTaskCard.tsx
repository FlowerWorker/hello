...
import SelectPriority from "./SelectPriority"; // Import the new component
// Icon imports
...
export default function AddTaskCard({ listName }: AddTaskCardProps) {
  ...
  const [selectedPriority, setSelectedPriority] = useState<string | null>(null); // Track selected priority
  ...
  return (
    <div
      className={`cursor-pointer flex flex-col bg-gray-50 sm:w-[330px] rounded-[20px] py-2 gap-3 shadow-left-heavy px-4 mb-3`}
    >
      {/* Label container */}
      ...
      <div>
        ...
        {/* Display selected priority if any */}
        {selectedPriority && (
      ...
      {/* Show SelectPriority modal when toggled */}
      {isPriorityModalOpen && (
        <SelectPriority
          togglePriority={() => setIsPriorityModalOpen(false)}
